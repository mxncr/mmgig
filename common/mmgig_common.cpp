...
namespace OGF {
    ...
    void mmgig_libinit::initialize() {

        Logger::out("Init") << "<mmgig>" << std::endl;

        //_____________________________________________________________

        gom_package_initialize(mmgig) ;

        ogf_register_grob_commands<OGF::MeshGrob,OGF::MeshGrobmmgcallsCommands>();
        ...
        module_info->set_name("mmgig");
        module_info->set_vendor("Maxence Reberol");
        module_info->set_version("3-1.x");
        module_info->set_info(
                "New package, created by Graphite Development Tools"
        );
        Module::bind_module("mmgig", module_info);

        Logger::out("Init") << "</mmgig>" << std::endl;
    }
    
    void mmgig_libinit::terminate() {
        Logger::out("Init") << "<~mmgig>" << std::endl;
        //_____________________________________________________________
        ...
        Module::unbind_module("mmgig");
        Logger::out("Init") << "</~mmgig>" << std::endl;
    }
    
    mmgig_libinit::mmgig_libinit() {
        increment_users();
    }
    ...
    void mmgig_libinit::increment_users() {
    ...
    void mmgig_libinit::decrement_users() {
        count_--;
        if(count_ == 0) {
            terminate();
        }
...
extern "C" void mmgig_API OGF_mmgig_initialize() {
    OGF::mmgig_libinit::increment_users();
}

extern "C" void mmgig_API OGF_mmgig_terminate() {
    OGF::mmgig_libinit::decrement_users();
}
